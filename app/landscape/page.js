  ...
  async function render() {
    ...
    try {
      ...
      for (let i = 0; i < r; i++) for (let j = 0; j < r; j++) {
        const v1 = mn + (mx - mn) * i / (r - 1), v2 = mn + (mx - mn) * j / (r - 1)
      ...
      for (let i = 0; i < r; i++) {
        ...
        for (let j = 0; j < r; j++) { z[i].push(out[port][i*r+j]); p1g[i].push(p1a[i*r+j]); p2g[i].push(p2a[i*r+j]) }
      }
      window.Plotly?.newPlot(plotRef.current, [{
        type: 'surface', z, x: p1g, y: p2g,
        colorscale: [['0','#0D1117'],['0.5','#1D4ED8'],['1','#22D3EE']],
        opacity: 0.92, contours: { z: { show: true, usecolormap: true, project: { z: true } } },
      }], {
      ...
      <div className="ls-controls">
        <div className="panel">
          ...
          <div className="panel-body" style={{ display:'flex', flexDirection:'column', gap:10 }}>
            ...
            <Field label="해상도" id="res" value={res} onChange={setRes} />
          </div>
        </div>
        <div className="panel">
