...
const PRESETS = [
  { label: '저속 2/2',  q1: 2,  q2: 2  },
  { label: '표준 5/5',  q1: 5,  q2: 5  },
  { label: '최대 10/10',q1: 10, q2: 10 },
]

export default function InversePage({ chip, branch, onBranch }) {
  ...
  async function run() {
    ...
    try {
      const r = await api.post('/solve_inverse', {
        chip_id: chip.chip_id,
        tq1: +tq1, tq2: +tq2,
        temp_device: +temp,
        m1_loading: +im1, m2_loading: +im2,
        p_max: +pmax, branch,
      })
      setResult(r)
    } catch (e) {
  ...
  return (
    <div className="page-wrap">
      <Topbar title="역해석 솔버" sub="INVERSE SOLVE · PRESSURE FINDER" branch={branch} onBranch={onBranch}/>
              ...
              <div className="preset-row">
                {PRESETS.map(p => (
                  <button key={p.label} className="preset-btn"
                    onClick={() => { setTq1(String(p.q1)); setTq2(String(p.q2)) }}>
                    {p.label}
                  </button>
                ))}
              </div>
            ...
            <div className="panel slide-in">
              ...
              <div className="panel-body">
                <div className="res-strip">
                  <div className="rs-item"><div className="rs-lbl">P1 권장</div><div className="rs-val">{(+result.p1).toFixed(2)} kPa</div></div>
                  <div className="rs-item"><div className="rs-lbl">P2 권장</div><div className="rs-val">{(+result.p2).toFixed(2)} kPa</div></div>
                  <div className="rs-item"><div className="rs-lbl">예상 Q1</div><div className="rs-val">{(+result.q1).toFixed(4)}</div></div>
                  <div className="rs-item"><div className="rs-lbl">예상 Q2</div><div className="rs-val">{(+result.q2).toFixed(4)}</div></div>
                </div>
              </div>
            </div>
          )}
